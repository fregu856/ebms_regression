...
class PostProcessor(nn.Module):
    ...
    def __init__(
        ...
        self.perform_filtering = perform_filtering
    ...
    def forward(self, x, boxes):
        ...
        for prob, boxes_per_img, image_shape in zip(
            ...
            if self.perform_filtering and not self.bbox_aug_enabled:  # If bbox aug is enabled, we will do it later
                boxlist = self.filter_results(boxlist, num_classes)
            if not self.perform_filtering:
                boxlist = self.remove_low_score_boxes(boxlist, num_classes)
...
def make_roi_box_un_post_processor(cfg):
    ...
    perform_filtering = cfg.MODEL.ROI_BOX_UN_HEAD.PERFORM_FILTERING
